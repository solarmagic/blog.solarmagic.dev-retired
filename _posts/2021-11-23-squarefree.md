...
약간의 관찰과 골드 문제 두개의 사전 지식이 있으면 풀 수 있는 문제이다. 하나 추가하면 이분 탐색(실버)에 관한 지식도 필요하다.

앞으로의 글은 두개의 골드 문제의 풀이를 담으면서 이 문제를 풀기 위한 관찰을 적어보았다.
...
좀 더 수학으로 정리하면 집합 $P = \{p | p$는 소수$\}$의 공집합이 아닌 모든 부분집합 $S$에 대해 $-1^{|S|}[N/q^2]$($q = \prod_{p\in S} p$)를 계산하면 $N$ 이하의 제곱 ㄴㄴ 수의 개수를 알 수 있다.
...
이 때 필요한 관찰은 $-1^{|S|}[N/q^2]$에서 **$q$가 $\sqrt N$보다 크다면 저 값은 항상 0이 된다는 것**이다. 원래 포함 배제 문제를 풀 때에는 모든 부분 집합 $S$를 찾고 그것에 해당하는 $q$를 만들어서 저 식을 계산했다면 이 문제에서는 반대로 $q$에 대해서 반복하면서 그에 해당하는 $S$가 있는지를 판단하는 것이다!
...
1. 집합 $P = \{p | p$는 소수$\}$의 공집합이 아닌 모든 부분집합 $S$에 대해 $-1^{|S|}[N/q^2]$($q = \prod_{p\in S} p$)를 계산하면 $N$ 이하의 제곱 ㄴㄴ 수의 개수를 알아낼 수 있다. 이를 $Q(N)$이라 하자.
2. 이 문제의 답이 될수 있는 최댓값을 $R = 2*10^{9}$ 라고 하자 최솟값은 $1$이다. $1$부터 $R$사이에서 $Q(x) = K$를 만족하는 최소의 $x$를 이분 탐색으로 알아낸다. 
3. $Q(x)$를 계산할 때 다 계산하면 양이 많으니 $q$의 후보가 최대 $\sqrt R$ 까지임을 이용해 $q$를 정했을때 $q$가 가능한 $q$인지 판단 및 $-1^{|S|}$의 값을 $q$를 소인수 분해해서 알아 낸다. 이를 전처리해서 알아내자.
